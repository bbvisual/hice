...
def compute_average_precision(pred, truth, tolerance=0, min_precision=0, plot_ax=None, plot_label=None, plot_raw_pr=True):
...
def compute_mAPs(truth, pred, tolerances=[0, 1, 2, 4], plot_pr=False, printed = False, stride = 1):
    ...
    if plot_pr:
        fig, axes = plt.subplots(
            ...
            figsize=(4 * len(tolerances), 4 * len(truth_by_label)))
    ...
    for i, tol in enumerate(tolerances):
        ...
        for j, (label, truth_for_label) in enumerate(
            ...
            ap = compute_average_precision(
                ...
                plot_ax=axes[i] if axes is not None and axes.ndim == 1
                   else axes[j, i] if axes is not None
                   else None)
    ...
    tab_rows = tabulate(rows, headers=header, floatfmt='0.2f')
    ...
    if plot_pr:
        for i, tol in enumerate(tolerances):
            for j, label in enumerate(sorted(truth_by_label.keys())):
                # Handle axes indexing regardless of dimensions
                if axes.ndim == 1:
                    ax = axes[i]  # 1D array (single label)
                else:
                    ax = axes[j, i]  # 2D array (multiple labels)
        ...
        # print(f"[INFO] PR curves saved as {fig_name}.png")

    sys.stdout.flush()
    return mAPs, tolerances, tab_rows, fig

def gaussian_window(window, sigma=None):
    """
    Create a Gaussian curve centered at 0 with peak value of 1.

    Args:
        window: int, distance from center (e.g., 2 means -2 to 2)
        sigma: float, standard deviation of Gaussian (default: window/2)

    Returns:
        dict: {position: gaussian_value}
    """
    if window <= 0:
        return {}

    if sigma is None:
        sigma = 3  # Default: make sigma half the window
