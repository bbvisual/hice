...
def make_temporal_shift(net, clip_len, mode='gsm', verbose=False):

    def _build_shift(net):
        if (mode == 'gsm') or (mode == 'gsf'):
            return GatedShift(net, n_segment=clip_len, n_div=4, mode=mode, verbose=verbose)
        else:
...
class GatedShift(nn.Module):
    def __init__(self, net, n_segment, n_div, mode='gsm', verbose=False):
